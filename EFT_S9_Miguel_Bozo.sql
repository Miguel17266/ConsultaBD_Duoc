...
/****Caso 1****/
-- Calcular Antiguedad de los clientes
SELECT 
    pnombre || ' ' || snombre || ' ' || appaterno || ' ' || apmaterno AS nombre_completo, 
    TRUNC(MONTHS_BETWEEN(SYSDATE, fecha_inscripcion) / 12) AS anios_inscripcion
FROM cliente;

--Filtrar clientes con más de 20 años de inscripción
SELECT 
    pnombre || ' ' || snombre || ' ' || appaterno || ' ' || apmaterno AS nombre_completo, 
    TRUNC(MONTHS_BETWEEN(SYSDATE, fecha_inscripcion) / 12) AS anios_inscripcion
FROM cliente
WHERE TRUNC(MONTHS_BETWEEN(SYSDATE, fecha_inscripcion) / 12) >= 20;

--Agrupar clientes por región y filtrar por los que tienen más de 20 años
SELECT 
    r.nombre_region AS region, 
    COUNT(c.numrun) AS total_clientes, 
    COUNT(CASE 
            WHEN TRUNC(MONTHS_BETWEEN(SYSDATE, c.fecha_inscripcion) / 12) >= 20 
            THEN 1 
         END) AS clientes_mas_20_anios
FROM cliente c
JOIN region r ON c.cod_region = r.cod_region
GROUP BY r.nombre_region
ORDER BY clientes_mas_20_anios ASC;

--Crear Vista ResumenClientesporRegion
CREATE VIEW ResumenClientesRegion AS
SELECT 
    r.nombre_region AS region, 
    COUNT(c.numrun) AS total_clientes, 
    COUNT(CASE 
...
SELECT * FROM ResumenClientesRegion;

--Crear índices para la optimización (se crean 2 índices, el primero para evitar un escaneo completo de la tabla cliente 
--y el segundo para mejorar el acceso a registros en consultas combinadas de Clientes y Región. Además se incluye la secuencia 
--para comprobar que los index funcional bien)
...
/*****Caso 2*****/
--Consultar transacciones con vvencimiento el segundo semestre
SELECT 
    tt.nombre_tptran_tarjeta AS tipo_transaccion, 
    ROUND(AVG(cttc.valor_cuota), 2) AS promedio_monto,
    SYSDATE AS fecha_emision
FROM cuota_transac_tarjeta_cliente cttc
JOIN transaccion_tarjeta_cliente ttc ON cttc.nro_transaccion = ttc.nro_transaccion
JOIN tipo_transaccion_tarjeta tt ON ttc.cod_tptran_tarjeta = tt.cod_tptran_tarjeta
WHERE EXTRACT(MONTH FROM cttc.fecha_venc_cuota) BETWEEN 6 AND 12
GROUP BY tt.nombre_tptran_tarjeta
ORDER BY promedio_monto ASC;

--Alternativa 1: Usar operador SET
SELECT 
    tt.nombre_tptran_tarjeta AS tipo_transaccion, 
    ROUND(AVG(cttc.valor_cuota), 2) AS promedio_monto,
    SYSDATE AS fecha_emision
...
JOIN transaccion_tarjeta_cliente ttc ON cttc.nro_transaccion = ttc.nro_transaccion
JOIN tipo_transaccion_tarjeta tt ON ttc.cod_tptran_tarjeta = tt.cod_tptran_tarjeta
WHERE EXTRACT(MONTH FROM cttc.fecha_venc_cuota) BETWEEN 6 AND 9
GROUP BY tt.nombre_tptran_tarjeta
...
--Alternativa 2: Usar Subconsulta
CREATE TABLE seleccion_tipo_transaccion_temp AS
SELECT 
...
SELECT * FROM seleccion_tipo_transaccion_temp;

--Actualizar tazas de intereses
...
WHERE tt.cod_tptran_tarjeta IN (
    SELECT stt.cod_tptran_tarjeta 
    FROM seleccion_tipo_transaccion_temp stt
);
...
WHERE cod_tptran_tarjeta IN (
    SELECT cod_tptran_tarjeta 
    FROM seleccion_tipo_transaccion_temp
);
...
Se deben encontrar las transacciones con cuotas que vencen entre junio y diciembre, calcular el promedio de sus montos
y actualizar la tasa de interés de las transacciones encontradas, lo que ayudará a gestionar de mejor manera la información
financiera y a ajustar automáticamente las codiciones de pago de los clientes

--2.¿Cuál es la información significativa que necesita para resolver el problema?
Para resolver el problema, se necesita la fecha de vencimiento de las cuotas para identificar las que ocurren entre junio y diciembre, 
los montos de las cuotas para calcular su promedio y los tipos de transacción asociados para actualizar la tasa de interés correspondiente. 
Esto permite gestionar y ajustar automáticamente las condiciones de las transacciones afectadas.

--3. ¿Cuál es el propósito de la solución que se requiere?
El propósito de la solución es identificar las transacciones relevantes según su fecha de vencimiento, calcular sus montos promedio y aplicar 
una actualización automática a la tasa de interés asociada. Esto permite mantener los datos financieros actualizados y mejorar la gestión de 
...
Primero, se crea una tabla temporal donde se almacenan los datos relevantes: el código y nombre del tipo de transacción junto con el promedio 
de los montos de las cuotas vencidas entre junio y diciembre. Luego, se utiliza esa tabla temporal para realizar una consulta que muestra la 
información requerida. Finalmente, se actualiza la tasa de interés de las transacciones presentes en la tabla temporal, aplicando una rebaja del 1%.

--5. Detalle los pasos, en lenguaje natural, necesarios para construir la alternativa que usa OPERADOR SET.
Primero, se realiza una consulta que selecciona los tipos de transacción y calcula el promedio de los montos de cuotas vencidas entre junio y septiembre. 
Luego, se hace otra consulta similar para las cuotas vencidas entre octubre y diciembre. Ambas consultas se combinan utilizando el operador UNION, 
que elimina los duplicados y devuelve un único conjunto de resultados. Finalmente, se ordenan los resultados por el promedio de montos de manera 
ascendente para facilitar su análisis.
*/
