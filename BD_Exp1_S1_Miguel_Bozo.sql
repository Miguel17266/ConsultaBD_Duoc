...
WHERE 
    fecha >= ADD_MONTHS(SYSDATE, -12) -- Filtra ventas del último año
GROUP BY 
    cliente_id
HAVING 
    SUM(cantidad * precio) = (
...
SELECT 
    ...
    ROUND(MONTHS_BETWEEN(SYSDATE, fecha_contratacion) / 12, 1) AS antiguedad_en_anios
FROM Empleados
WHERE ROUND(MONTHS_BETWEEN(SYSDATE, fecha_contratacion) / 12, 1) > 10
ORDER BY antiguedad_en_anios DESC;
